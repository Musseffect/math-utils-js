...
class QR {
    ...
    Q: Matrix;
    R: Matrix;
    protected compute(numIters: number) {
        assert(this.A.isSquare(), "Expected square matrix");
    }
    constructor(A: Matrix, numIters: number) {
        this.compute(numIters);
    }
    // computes real port of eigenvalues
    // todo: Givens rotations, complex eigenvalues
    static calcEigenvalues(A: Matrix, numIters: number, tolerance: number): number[] {
